/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package cursoBasicodeJava.aula15.execicio11;

import java.util.Scanner;

/**
 * As Organizações Tabajara resolveram dar um aumento de salário aos seus
 * colaboradores e lhe contraram para desenvolver o programa que calculará os
 * reajustes. o Faça um programa que recebe o salário de um colaborador e o
 * reajuste segundo o seguinte critério, baseado no salário atual: o salários
 * até R$ 280,00 (incluindo) : aumento de 20% o salários entre R$ 280,00 e R$
 * 700,00 : aumento de 15% o salários entre R$ 700,00 e R$ 1500,00 : aumento de
 * 10% o salários de R$ 1500,00 em diante : aumento de 5% Após o aumento ser
 * realizado, informe na tela: o o salário antes do reajuste; o o percentual de
 * aumento aplicado; o o valor do aumento; o o novo salário, após o aumento.
 *
 * @author Carlos
 */
public class AumentoSalario {

    public static void main(String[] args) {
        Scanner ler = new Scanner(System.in);
        double salarioVelho, salarioNovo, porcetagemAumenta, aumento;
        System.out.println("Qual e o seu salario atual?");
        salarioVelho = ler.nextDouble();
        if (salarioVelho <= 280.00) {
            porcetagemAumenta = 0.20;
            aumento = salarioVelho * porcetagemAumenta;
            salarioNovo = salarioVelho + aumento;
            System.out.println("O seu salario atual: R$ " + salarioVelho
                    + "\nTaxa Aliquota de Aumento: " + (porcetagemAumenta * 100)
                    + "\nAumentara para R$ " + aumento
                    + "%\nO seu Novo Salario: R$ " + salarioNovo);
        } else if (salarioVelho <= 700.00) {
            porcetagemAumenta = 0.15;
            aumento = salarioVelho * porcetagemAumenta;
            salarioNovo = salarioVelho + aumento;
            System.out.println("O seu salario atual: R$ " + salarioVelho
                    + "\nTaxa Aliquota de Aumento: " + (porcetagemAumenta * 100)
                    + "\nAumentara para R$ " + aumento
                    + "%\nO seu Novo Salario: R$ " + salarioNovo);
        } else if (salarioVelho <= 1500.00) {
            porcetagemAumenta = 0.10;
            aumento = salarioVelho * porcetagemAumenta;
            salarioNovo = salarioVelho + salarioVelho * aumento;
            System.out.println("O seu salario atual: R$ " + salarioVelho
                    + "\nTaxa Aliquota de Aumento: " + (porcetagemAumenta * 100)
                    + "\nAumentara para R$ " + aumento
                    + "%\nO seu Novo Salario: R$ " + salarioNovo);
        } else {
            porcetagemAumenta = 0.05;
            aumento = salarioVelho * porcetagemAumenta;
            salarioNovo = salarioVelho + aumento;
            System.out.println("O seu salario atual: R$ " + salarioVelho
                    + "\nTaxa Aliquota de Aumento: " + (porcetagemAumenta * 100)
                    + "\nAumentara para R$ " + aumento
                    + "%\nO seu Novo Salario: " + salarioNovo);
        }
    }
}
